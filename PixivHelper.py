...
def make_filename(nameFormat: str,
    ...
    if nameFormat.find("%works_date_fmt") > -1:
        ...
        for tr in to_replace:
            date_format = re.findall("{([^\}]+)}", tr)
            nameFormat = nameFormat.replace(tr, imageInfo.worksDateDateTime.strftime(date_format[0]))
    ...
    if nameFormat.find("%date_fmt") > -1:
        ...
        for tr in to_replace2:
            date_format2 = re.findall("{([^\}]+)}", tr)
            nameFormat = nameFormat.replace(tr, datetime.today().strftime(date_format2[0]))
